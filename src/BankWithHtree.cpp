...
void BankWithHtree::Initialize(int _numRowSubArray, int _numColumnSubArray, long long _capacity,
		...
		//cout << "--------------------------------------------------------------------\n\n";
	if (_numRowSubArray == 0 || _numColumnSubArray == 0){
		invalid = true;
		initialized = true;
		return;
	}
	...
	levelHorizontal = (int)(log2(numColumnSubArray)+0.1);
	levelVertical = (int)(log2(numRowSubArray)+0.1);
	...
	int v = levelVertical;
	int rowToActive = numActiveSubArrayPerColumn;
	int columnToActive = numActiveSubArrayPerRow;
	...
	if (memoryType == MemoryType::data) {		/* Data array */
		...
		/* Consider the case if each subarray is a cache data array that contains multiple ways */
	...
	subarray.Initialize(numRowMat, numColumnMat, numAddressBitToRoute, subarrayBlockSize,
	...
	if (!initialized) {
		...
		while (v > h) {
			if (v == levelVertical - 1)
				lengthVerticalWire[v] = subarray.height / 2;
			else
		...
		while (v >= 0) {
			...
			if (h == levelHorizontal - 1) {
				lengthHorizontalWire[h] = subarray.width;
...
void BankWithHtree::CalculateLatencyAndPower() {
	if (!initialized) {
		...
		//writeLatency = ((subarray.numRowMat * subarray.numColumnMat * subarray.mat.numColumn) /  subarray.mat.writeLatency) / area;
		//writeLatency = (0.5 * subarray.mat.readLatency + 0.5 * subarray.mat.readLatency_bidir)/2;
		...
		setLatency = subarray.setLatency;
		resetDynamicEnergy = subarray.resetDynamicEnergy * numActiveSubArrayPerRow * numActiveSubArrayPerColumn;
		setDynamicEnergy = subarray.setDynamicEnergy * numActiveSubArrayPerRow * numActiveSubArrayPerColumn;
